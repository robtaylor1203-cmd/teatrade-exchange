-- Fix division by zero edge case when closing index positions
-- The v_close_margin and leverage logic fails when v_existing_qty matches p_quantity precisely.
        ...
        ELSE
            ...
            ELSE
                UPDATE index_positions SET quantity = v_new_qty,
                    margin_used = GREATEST(0, COALESCE(v_position.margin_used, 0) - v_close_margin), updated_at = NOW()
                    WHERE id = v_position.id;
            END IF;
        END IF;
