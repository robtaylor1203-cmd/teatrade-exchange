-- ═══════════════════════════════════════════════════════════════════════════════
-- FIX STOP-OUT FORMULA — use POSITION HEALTH instead of MARGIN LEVEL
--
-- Old formula: equity / used_margin  (penalises low free cash even with
--              tiny position losses → premature liquidation)
--
-- New formula: (used_margin + unrealized_pnl) / used_margin
--              Tracks how much of the *invested capital* remains.
--              40% remaining → margin call
--               5% remaining → auto-liquidation
--
-- Also: hard equity floor — if equity <= 0, always liquidate.
-- ═══════════════════════════════════════════════════════════════════════════════
...
BEGIN
    ...
    LOOP
        ...
        -- ────────────────────────────────────────────────────────────────
        v_position_health := (v_used_margin + v_total_pnl) / v_used_margin;
        ...
        -- ── STOP-OUT (position health <= 5%) ──────────────────────────
        IF v_position_health <= v_stop_level THEN
            ...
            -- Liquidate all index positions (with P&L)
            FOR v_pos IN
                SELECT ip.*, i.teas AS idx_teas, i.multiplier
                FROM index_positions ip
                JOIN indexes i ON i.symbol = ip.index_symbol
            ...
            INSERT INTO margin_notifications (user_id, type, trading_mode, equity, used_margin, margin_level, message)
                VALUES (v_user.user_id, 'STOP_OUT', v_user.trading_mode,
                        v_equity, v_used_margin, v_position_health * 100,
                        'All positions liquidated — invested capital fell below '
                        || ROUND(v_stop_level * 100) || '% remaining.');

            v_liquidated := v_liquidated + 1;

        -- ── MARGIN CALL (position health <= 40%) ──────────────────────
        ELSIF v_position_health <= v_call_level THEN
            SELECT EXISTS (
            ...
            IF NOT v_already_warned THEN
                INSERT INTO margin_notifications (user_id, type, trading_mode, equity, used_margin, margin_level, message)
                    VALUES (v_user.user_id, 'MARGIN_CALL', v_user.trading_mode,
                            v_equity, v_used_margin, v_position_health * 100,
                            'Positions have lost ' || ROUND((1 - v_position_health) * 100)
                            || '% of invested capital. Close positions or deposit funds to avoid '
                            || 'liquidation at ' || ROUND(v_stop_level * 100) || '% remaining.');
